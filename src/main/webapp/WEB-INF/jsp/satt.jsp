...
//stmt1=con.prepareStatement("select count(SHRUTI) from pattendance WHERE SHRUTI='present'" );
  stmt1=con.prepareStatement("select (select count(SHRUTI) from pattendance WHERE SHRUTI='present' )as tp,(select count(shruti) from pattendance WHERE shruti='absent')as na from pattendance  ");
...
%>
  ...
  %>
 <h1 > TOTAL SESSIONS YOU ATTENDED =   <%=A[3]%>;</h1>


 
  <h1 > NUMBER OF SESSION YOU NEED TO ATTEND TO GET 75% ATTENDANCE = <%=A[4]%> </h1>  
   
<canvas id="myChart" style="width:30%;max-width:400px;max-height: 300px;left-margin:80%; right-margin:20%;  background: whitesmoke  "></canvas>
<script>
